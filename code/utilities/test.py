...
vector_store = RedisExtended(redis_url=self.vector_store_full_address, index_name=self.index_name, embedding_function=self.embeddings.embed_query) if vector_store is None else vector_store   


embeddings = OpenAIEmbeddings(model=os.getenv('OPENAI_EMBEDDINGS_ENGINE_DOC', "text-embedding-ada-002"), chunk_size=1) 

vector_store_protocol = 'redis://'
vector_store_address = 'newklfnwdkogbewio-redis.eastus.azurecontainer.io'
vector_store_port = '6379'
vector_store_full_address =   f"{vector_store_protocol}{vector_store_address}:{vector_store_port}"
index_name = "embeddings"

RedisExtended = RedisExtended(redis_url=vector_store_full_address, index_name=index_name, embedding_function=embeddings.embed_query)